...
public class Admin {
	...
	private void initialize(final User user) {
		...
		JButton btnNewButton = new JButton("Cab");
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				EventQueue.invokeLater(new Runnable() {
					public void run() {
						try { frameAdmin.setVisible(false);
							Admincab window = new Admincab(user);
							window.frameAdmincab.setVisible(true);
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
		...
		btnNewButton.setBounds(163, 104, 89, 23);
